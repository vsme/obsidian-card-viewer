...
export default class CardViewerPlugin extends Plugin {
  ...
  private async registerAllProcessors(): Promise<void> {
    ...
    for (const processor of processors) {
      await safeExecute(processor.fn, this.errorHandler, processor.name);
    }
  }
  ...
  private createHtmlContainer(el: HTMLElement): HTMLElement {
    const container = el.createEl("div", {
      cls: "card-viewer-html-container",
    });

    // 添加类型标识
    const header = container.createEl("div", {
      cls: "card-viewer-html-header",
    });

    const typeEl = header.createEl("span", {
      cls: "card-viewer-html-type",
      text: "HTML",
    });
  ...
  private isEmptyHtmlContent(content: string): boolean {
    ...
    // 定义媒体标签（需要排除的标签）
    const mediaTags =
      "img|video|audio|source|track|embed|object|iframe|canvas|svg|picture";

    // 检查是否只包含空的HTML标签（排除媒体标签，不含样式、类或ID属性）
    const emptyTagPatterns = [
      new RegExp(
        `^\\s*<(?!(${mediaTags})\\b)\\w+(?:\\s+(?!style|class|id)[^=]*(?:="[^"]*")?)*>\\s*<\/(?!(${mediaTags})\\b)\\w+>\\s*$`,
        "i"
      ),
      new RegExp(
        `^\\s*<(?!(${mediaTags})\\b)\\w+(?:\\s+(?!style|class|id)[^=]*(?:="[^"]*")?)*\\/>\\s*$`,
        "i"
      ),
    ];

    return emptyTagPatterns.some(
      (pattern) =>
   ...
   * 处理图片网格代码块
   */
  private async processImgsBlocks(
    el: HTMLElement,
    ctx: MarkdownPostProcessorContext
  ): Promise<void> {
   ...
   * 处理单个图片网格代码块
   */
  private async processImgsBlock(
    codeBlock: HTMLElement,
    ctx: MarkdownPostProcessorContext
  ): Promise<void> {
    const codeContent = codeBlock.textContent || "";
    ...
    gridContainer.className = "card-viewer-imgs-grid-container";
